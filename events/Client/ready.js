...
const GuldModel = require("../../commands/schemas/guild.js");
...
module.exports = async (client) => {
    ...
    setInterval(async () => {
        client.guilds.cache.forEach(async guild => {
            const check = GuldModel.findOne({ GuildId: guild.id}, async(err, data) => {
                  if(err) throw err;
                 
                  if (check) {
                    var server = client.guilds.cache.get(check)
                    let Cid = await dbq.get(`channel_${server?.id}_clear`)
                    var Cch = client.channels.cache.get(Cid?.id)
                    if (Cch) {
                        try {
                            var messagecount = "2";
                            const msgs = await Cch.messages.fetch({ limit: 100 }).then(messages => Cch.bulkDelete(messagecount, true));
            
                            //------------------code colors---------------------\\
                            let x = 0;
                            let y = 150; //ثابت نكفى
            
                            if (server.roles.cache.filter(role => !isNaN(role.name) && !role.name.includes('.')).size <= 0) return;
                            let sizee = server.roles.cache.filter(role => !isNaN(role.name) && !role.name.includes('.')).size;
                            server.roles.cache.filter(role => !isNaN(role.name) && !role.name.includes('.')).sort((b1, b2) => b1.name - b2.name).forEach(() => {
                            ...
                            var photo = await dbq.get(`img_${server.id}_colors`)
                            if (photo == null) {
                            ...
                            server.roles.cache.filter(role => !isNaN(role.name)).sort((b1, b2) => b1.name - b2.name).forEach(role => {
                            ...
                            const attachment = new Discord.MessageAttachment(colrsList.toBuffer(), 'img.png');
            
                            Cch.send({ files: [attachment] }).catch(m => { return; });
    ...
    client.guilds.cache.forEach(async guild => {
      const check = GuldModel.findOne({ GuildId: guild.id}, async(err, data) => {
            ...
            if (check) {
                ...
                const guildd = await client.guilds.cache.get(check)
                if (guildd) {
                    let data = await dbq.get(`voiceRoom ${guildd.id} online`);
                    if (data == null) return console.log('null');
                    if (!data) return
                    const room = guildd.channels.cache.get(`${data}`);
                    if (room) {
                        try {
                            setInterval(async () => {
                                let namev = await dbq.get(`voiceRoom_${guildd.id}_nameEdit`);
                                if (!namev) return;
                                let Online = await guildd.members.cache.filter(m => m.voice.channel);
                                room.setName(namev.replace("[00]", `${Online.size}`));
                            }, 5000);
                        } catch (e) {
                            console.log(e.stack)
                        }
